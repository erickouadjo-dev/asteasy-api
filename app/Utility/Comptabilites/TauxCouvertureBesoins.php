...
class TauxCouvertureBesoins{
    ...
    public static function generer(Request $request){
        try{
            ...
            $rules = [
              ...
              'type' => 'required|string|in:i_taux_couverture_besoins2',
            ];
            ...
            $produit= self::totalProduit($inputs);
            $charge= self::totalCharge($inputs);
            $decompte=self::decomptesPayes($inputs);
            if ($produit != 0 && $charge != 0) {
              # code...
              $result["valeur"]=($decompte/($produit-$charge))*100;
            }
    ...
    public static function decomptesPayes($inputs)
    {
      ...
      try{
        ...
        Log::error('TauxCouvertureBesoins::decomptesPayes a échoué avec le message ' . $e->getMessage(), ['trace'=>$e->getTraceAsString()]);
      }
    }

    public static function totalProduit($inputs)
    {
      # code...
      try{
        $total_montant_produit=0;
        $produits= ComptabiliteGlobale::join("plan_comptes","comptabilite_globales.compte","=","plan_comptes.id")
        ...
        ->Where("plan_comptes.compte","LIKE","70%")
        ->orWhere("plan_comptes.compte","LIKE","77%")
        ...
        if(count($produits)){
          foreach ($produits as $produit) {
            # code...
            $total_montant_produit+=$produit->montant_facture_produit;
          }
        }

        return $total_montant_produit;
      }catch(\Exception $e){
        Log::error('TauxCouvertureBesoins::totalProduit a échoué avec le message ' . $e->getMessage(), ['trace'=>$e->getTraceAsString()]);
      }
    }
    ...
    public static function totalCharge($inputs)
    {
      ...
      try{
        ...
        Log::error('TauxCouvertureBesoins::totalCharge a échoué avec le message ' . $e->getMessage(), ['trace'=>$e->getTraceAsString()]);
      }
    }
}
