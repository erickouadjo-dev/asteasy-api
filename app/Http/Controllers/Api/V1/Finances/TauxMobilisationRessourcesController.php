...
use App\Utility\Comptabilites\TauxMobilisationRessources;
use App\Utility\PolicyResources\Finances\Indicateurs as IndicateursResource;
use Log;


class TauxMobilisationRessourcesController extends Controller
{
    //
    /**
  ...
  public function index(Request $request)
  {
      try{
          ...
          $result = TauxMobilisationRessources::generer($request);
          return response()->json($result, $result['code_http']);
      }catch(\Illuminate\Auth\Access\AuthorizationException $e){
          Log::error('Finances/TauxMobilisationRessourcesController@index a échoué avec le message ' . $e->getMessage(), ['trace'=>$e->getTraceAsString()]);
