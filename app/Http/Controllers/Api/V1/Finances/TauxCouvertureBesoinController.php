...
use App\Utility\Comptabilites\TauxCouvertureBesoins;
use App\Utility\PolicyResources\Finances\Indicateurs as IndicateursResource;
use Log;


class TauxCouvertureBesoinController extends Controller
{
    //
        /**
  ...
  public function index(Request $request)
  {
      try{
          ...
          $result = TauxCouvertureBesoins::generer($request);
          return response()->json($result, $result['code_http']);
      }catch(\Illuminate\Auth\Access\AuthorizationException $e){
          Log::error('Finances/TauxCouvertureBesoinController@index a échoué avec le message ' . $e->getMessage(), ['trace'=>$e->getTraceAsString()]);
