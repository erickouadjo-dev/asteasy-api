# Documentation API - Récurrences

## Aperçu

- Racine endpoints : `https://asteasy.deepinovia.com/api/api`
- Préfixe API : `/v1`
- Ressource : `/recurrences`
- Middleware de groupe : `cors`, `multi_authentication`
- **Isolation Multi-Tenant** : Non applicable (la table `TB_RECURRENCE` ne contient pas de colonne `ENTREPRISE_ID` et n'est pas cloisonnée par locataire).
- Policy :
  - Lecture (`index`, `show`) : tout utilisateur authentifié.
  - Écriture (`store`, `update`, `destroy`) : utilisateur de type `ADMIN` ou `POWER_USER`.

## Structure de la ressource Récurrence

```json
{
  "ID": 1,
  "LIBELLE": "Annuelle",
  "DESCRIPTION": "L action se répète chaque année.",
  "IS_DELETE": false,
  "CREATED_BY": 2,
...
Exemple :
```bash
curl -X GET "https://asteasy.deepinovia.com/api/api/v1/recurrences?per_page=10&page=1&search=Annuelle" \
...
Body JSON :
- `LIBELLE` (requis, string, max 255, unique dans `TB_RECURRENCE`)
- `DESCRIPTION` (requis, string)

Exemple :
```bash
curl -X POST "https://asteasy.deepinovia.com/api/api/v1/recurrences" \
  -H "Authorization: Bearer <token_admin_ou_power_user>" \
  -H "Content-Type: application/json" \
  -d '{
    "LIBELLE": "Mensuelle",
    "DESCRIPTION": "L action se répète chaque mois."
  }'
```
...
Exemple :
```bash
curl -X GET "https://asteasy.deepinovia.com/api/api/v1/recurrences/2" \
  -H "Authorization: Bearer <token>"
```
...
### 4) Mettre à jour une récurrence
- Méthode : `PUT`
- URL : `/v1/recurrences/{id}`
- Autorisation : `ADMIN` ou `POWER_USER`
- Content-Type : `application/json`
...
- `LIBELLE` (optionnel, string, max 255, unique dans `TB_RECURRENCE` sauf pour cet ID)
- `DESCRIPTION` (optionnel, string)

Exemple :
```bash
curl -X PUT "https://asteasy.deepinovia.com/api/api/v1/recurrences/2" \
  -H "Authorization: Bearer <token_admin_ou_power_user>" \
  -H "Content-Type: application/json" \
  -d '{
...
Exemple :
```bash
curl -X DELETE "https://asteasy.deepinovia.com/api/api/v1/recurrences/2" \
  -H "Authorization: Bearer <token_admin_ou_power_user>"
```
...
- Contrôleur : `app/Http/Controllers/Api/V1/RecurrencesController.php`
- Modèle : `app/Models/Recurrence.php`
- Stratégie d'autorisation (Policy) : `app/Policies/RecurrencesPolicy.php`
