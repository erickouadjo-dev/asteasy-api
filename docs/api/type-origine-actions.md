# Documentation API - Types d'origine d'action
...
## Structure de la ressource Type d'origine d'action
...
### 1) Lister les types d'origine d'action
- Méthode : `GET`
- URL : `/v1/type-origine-actions`
...
Exemple :
```bash
curl -X GET "https://asteasy.deepinovia.com/api/api/v1/type-origine-actions?per_page=10&page=1&search=Audit" \
...
### 2) Créer un type d'origine d'action
- Méthode : `POST`
- URL : `/v1/type-origine-actions`
...
Exemple :
```bash
curl -X POST "https://asteasy.deepinovia.com/api/api/v1/type-origine-actions" \
...
### 3) Récupérer un type d'origine d'action
- Méthode : `GET`
- URL : `/v1/type-origine-actions/{id}`
- Autorisation : utilisateur authentifié

Exemple :
```bash
curl -X GET "https://asteasy.deepinovia.com/api/api/v1/type-origine-actions/2" \
  -H "Authorization: Bearer <token>"
```
...
### 4) Mettre à jour un type d'origine d'action
- Méthode : `PUT`
- URL : `/v1/type-origine-actions/{id}`
- Autorisation : `ADMIN` ou `POWER_USER`
- Content-Type : `application/json`
...
Exemple :
```bash
curl -X PUT "https://asteasy.deepinovia.com/api/api/v1/type-origine-actions/2" \
  -H "Authorization: Bearer <token_admin_ou_power_user>" \
  -H "Content-Type: application/json" \
  -d '{
...
### 5) Supprimer un type d'origine d'action
- Méthode : `DELETE`
- URL : `/v1/type-origine-actions/{id}`
- Autorisation : `ADMIN` ou `POWER_USER`
...
Exemple :
```bash
curl -X DELETE "https://asteasy.deepinovia.com/api/api/v1/type-origine-actions/2" \
  -H "Authorization: Bearer <token_admin_ou_power_user>"
```
...
- Contrôleur : `app/Http/Controllers/Api/V1/TypeOrigineActionsController.php`
- Modèle : `app/Models/TypeOrigineAction.php`
- Stratégie d'autorisation (Policy) : `app/Policies/TypeOrigineActionsPolicy.php`
